...
public class ProjectsDao {
    ...
    public void create(Projects project) {
        ...
        String sql = """
                    ...
                    (user_id, projName, projDescription, techStackIds, status, startDate, endDate, createdAt, updatedAt)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """;
        ...
        try (
            ...
            ps.setString(2, project.projName);
            ps.setString(3, project.projDescription);
            ps.setString(4, toJson(project.techStackIds));
            ps.setString(5, project.status);
            ps.setObject(6, project.startDate);
            ps.setObject(7, project.endDate);
            ps.setTimestamp(8, Timestamp.valueOf(project.createdAt));
            ps.setTimestamp(9, Timestamp.valueOf(project.updatedAt));
    ...
    public List<Projects> findByUserId(Long userId) {
        String sql = "SELECT * FROM projects WHERE user_id = ? ORDER BY createdAt DESC";
        ...
        try (
            ...
            throw new RuntimeException("Failed to fetch projects by user id", e);
        }
    ...
    public Projects findById(Long id, Long userId) {
    ...
    // UPDATE
    // -------------------------------------------------------------------------- >
    public void update(Projects project, Long userId) {
